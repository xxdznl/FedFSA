...
class ClientMoFedSAM(ClientFedAvg):
    ...
    def train_cnn(self,epoch,last):
        self.model.train()
        # ##### uncomment code below to switch to MoFedSAM + FSA #######
        # params_list = [param.clone().detach() for param in self.model.parameters()]
        # ##### uncomment code above to switch to MoFedSAM + FSA #######
        ...
        if last:
            ...
            # MoFedSAM and with FSA. MoFedSAM solely when rho_larger == rho_default
...
class ServerMoFedSAM(ServerFedAvg):
